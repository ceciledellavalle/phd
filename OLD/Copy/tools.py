...
def compute_wass_results(test_config, dataset, path_iRestNet):
    """
    Computes the mean SSIM of the blurred and restored images.
    Parameters
    ----------
        test_config   (str): test configuration from {'NoKernel'}
        dataset       (str): name of the test set
        path_iRestNet (str): path to images restored with iRestNet
    Results
    -------
        (float): average Wasserstein distance of the blurred images
        (float): average Wasserstein distance of the restored images
    """
    ### test configuration
    ...
    path_true        = os.path.join('Datasets','Groundtruth', dataset)
    ...
    wass_blurred, wass_iRestNet = np.zeros(len(file_names)),np.zeros(len(file_names))
    ...
    for i in range(0,len(file_names)):
        ...
        ### compute ssim
        wass_blurred[i]       = my_compare_OT(x_true, x_blurred)
        wass_iRestNet[i]      = my_compare_OT(x_true, x_iRestNet)

    return np.mean(wass_blurred), np.mean(wass_iRestNet)
